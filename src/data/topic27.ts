...
export const topic27Data: TopicData = {
  ...
  storytelling: {
    ...
    technicalConnection: "**TM Forum TMF642 Alarm Management API** (REST/JSON): GET /tmf-api/alarmManagement/v4/alarm?severity=critical&state=active returns alarm array. Alarm schema: { id, alarmType, perceivedSeverity (critical|major|minor|warning), probableCause, specificProblem, alarmRaisedTime (ISO8601), alarmChangedTime, state (raised|updated|acknowledged|cleared), sourceSystemId, affectedService[], alarmedObject {id, @type, @referredType}, ackState (unacknowledged|acknowledged), ackUserId, ackTime }. POST /tmf-api/alarmManagement/v4/alarm/{id}/acknowledge with body {ackUserId, ackTime, ackSystemId}. **TMF628 Performance Management API**: GET /tmf-api/performanceManagement/v4/performanceMeasurement?resourceId=Router-X&granularity=15min returns measurement array with { id, granularity (15min|1hour|1day), measurementTime, performanceMetric[{name: 'ifInOctets', value, unit: 'bytes'}] }. **TMF639 Resource Inventory API**: GET /tmf-api/resourceInventoryManagement/v4/resource?resourceType=PhysicalDevice returns device inventory. Resource schema: { id, name, resourceType (LogicalResource|PhysicalResource), resourceSpecification {id, name}, resourceCharacteristic[{name: 'serialNumber', value}], resourceRelationship[{type: 'connectedTo', resource: {id, @referredType}}] }. **RFC 8040 RESTCONF NBI**: URL /restconf/data/{module}:{resource}. Example: GET /restconf/data/ietf-alarms:alarms/alarm-list/alarm={alarm-id}. Response: application/yang-data+json with YANG-modeled alarm data. **RFC 6749 OAuth 2.0 Authentication**: Client credentials grant flow - POST /oauth2/token with grant_type=client_credentials, client_id, client_secret. Response: {access_token, token_type: 'Bearer', expires_in: 3600}. Subsequent requests: Authorization: Bearer {access_token}. **RFC 7235 Bearer Token**: HTTP Authorization header format - Authorization: Bearer <token>. Server validates token signature (JWT RS256/HS256), checks expiry (exp claim), extracts subject (sub claim) and scopes. **Webhook Push Notification**: NMS POSTs alarm JSON to registered OSS callback URL. OSS registers webhook: POST /nbi/subscriptions body {callbackUrl: 'https://oss.example.com/alarms/webhook', filter: {severity: ['critical', 'major']}}. NMS stores subscription, sends POST to callbackUrl when matching alarm raised. **Rate Limiting**: Per-client quota: X-RateLimit-Limit: 1000 (requests/hour), X-RateLimit-Remaining: 847, X-RateLimit-Reset: 1640000000 (Unix epoch). 429 Too Many Requests + Retry-After: 3600 when exceeded."
  },
  mathModelling: {
