...
export const topic36Data: TopicData = {
  ...
  storytelling: {
    ...
    technicalConnection: "**RFC 6933 Entity MIB v4**: entPhysicalTable (1.3.6.1.2.1.47.1.1.1) - entPhysicalDescr, entPhysicalClass (chassis/module/port), entPhysicalSerialNum. SNMP GET-NEXT walk retrieves inventory. **RFC 8639 YANG-Push**: On-change subscription - <establish-subscription><stream>YANG-Push</stream><encoding>encode-json</encoding><on-change><dampening-period>100</dampening-period></on-change></establish-subscription>. Device pushes notification only when config changes → event-driven vs polling. **OpenTelemetry Protocol (OTLP)**: ExportMetricsServiceRequest{resource_metrics[{resource: {attributes: [{key:'service.name', value:'router-1'}]}, scope_metrics[{scope, metrics[{name:'interface.bytes.in', gauge:{data_points[{time_unix_nano, as_int, attributes[{key:'interface', value:'GigE0/1'}]}]}}]}]}]}. High-cardinality: millions of unique (interface, vlan, customer_id) combinations. **RFC 5424 Syslog vs Observability Logs**: Syslog: unstructured text → regex parsing required. Observability: structured JSON - {\"@timestamp\":\"2025-01-15T14:32:05Z\",\"severity\":\"warning\",\"event\":\"bgp_state_change\",\"peer\":\"10.1.1.1\",\"old_state\":\"Established\",\"new_state\":\"Idle\",\"reason\":\"hold_timer_expired\"}. **gNMI SubscribeMode**: ONCE (single read), POLL (on-demand → SubscriptionList with suppress_redundant=true), STREAM (continuous push with sample_interval or on_change). **W3C Trace Context**: Distributed tracing across services - API Gateway (span 1: 50ms) → Firewall VNF (span 2: 12ms) → Application Pod (span 3: 200ms). Parent-child relationship via parent_span_id. **MTTD Formulas**: Monitoring MTTD = T_poll/2 + T_alert_processing. Observability MTTD = T_telemetry_latency + T_anomaly_detection. ΔMTTD = MTTD_monitor - MTTD_observe. **Prometheus vs RRD Storage**: Prometheus: label-value pairs → inverted index. SELECT metric WHERE label1='value1' AND label2='value2'. RRD: fixed schema, one file per metric → no arbitrary label queries. High-cardinality (customer_id, trace_id) requires column-store (ClickHouse, BigTable) not RRD."
  },
  mathModelling: {
