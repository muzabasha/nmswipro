...
export const topic17Data: TopicData = {
  ...
  storytelling: {
    ...
    technicalConnection: "NETCONF RPC operations (RFC 6241): get-config (source: running/candidate/startup, optional subtree/xpath filter) retrieves configuration data. get (no source, optional filter) retrieves config + operational state data. edit-config (target: candidate/running, default-operation: merge/replace/none, per-node operation attribute: merge/replace/create/delete/remove) modifies configuration. commit applies candidate to running. discard-changes reverts candidate. lock/unlock (target datastore) provides exclusive access. validate (source: candidate/running) checks schema validity without committing. copy-config (source, target) duplicates datastores. delete-config removes a datastore. kill-session (session-id) terminates another session forcibly."
  },
  mathModelling: {
