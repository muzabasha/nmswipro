...
export const topic44Data: TopicData = {
  ...
  virtualLab: {
    description: "You are a 5G slice operator managing 3 network slices (eMBB, URLLC, mMTC) on shared infrastructure. Your task: compare static slicing (each slice gets fixed capacity) vs dynamic slicing (ONAP-based demand-responsive allocation) to find the capacity efficiency gain. Adjust the traffic load percentage and number of slice types. The chart shows wasted capacity under static allocation — unused resources that dynamic slicing could reclaim. Find the load level where dynamic slicing saves the most infrastructure cost.",
    interpretation: "At 20% traffic load with 3 slices, static allocation wastes 92% of capacity — each slice reserves 33% but collectively only uses 17%. Dynamic slicing reclaims unused capacity, matching actual demand. At 90% load, static allocation still wastes ~15% because peak traffic for different slices rarely coincides — dynamic slicing temporarily borrows from low-demand slices to prevent SLA breaches. The efficiency gain (up to 47% at typical load levels) is the primary economic justification for ONAP-based slice automation. Use this lab to calculate the ROI of dynamic slice orchestration for your deployment.",
    parameters: [
