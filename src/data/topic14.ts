...
export const topic14Data: TopicData = {
  ...
  storytelling: {
    ...
    technicalConnection: "RFC 6020 (YANG 1.0) §4 defines module structure: **module** example-module { **yang-version** 1.1; **namespace** \"http://example.com/ns/example\"; **prefix** \"ex\"; **import** ietf-inet-types { prefix inet; }; **organization** \"Example Corp\"; **revision** 2024-01-15 { description \"Initial version\"; }; **container** interfaces { **list** interface { **key** \"name\"; **leaf** name { type string; } **leaf** type { type identityref { base interface-type; }} **leaf** enabled { type boolean; default true; } **leaf-list** ipv4-address { type inet:ipv4-address; } } } }. **YANG Statement Hierarchy**: module > {container|list} > {leaf|leaf-list|container|list}. **Data Types (18 built-in)**: binary, bits, boolean, decimal64, empty, enumeration, identityref, instance-identifier, int8/16/32/64, uint8/16/32/64, leafref, string, union. **Container**: grouping node with child statements, no value itself. Example: container system { leaf hostname {...} leaf domain-name {...} }. **List**: repeating structure with **key** leafs as primary key (single or composite). Example: list interface { key \"name\"; ... } allows /interfaces/interface[name='eth0']. **Leaf**: atomic data element with type. **config true** = writable via edit-config; **config false** = read-only operational state (retrieved via get, not editable). **Grouping**: reusable schema template. **typedef**: custom type definition (e.g., typedef percent { type uint8 { range 0..100; } }). **Augment**: extends external module's tree (e.g., augment \"/if:interfaces/if:interface\" { leaf custom-param {...} }). **Must/When constraints**: XPath boolean expressions. **must** \"count(../interface) <= 32\" restricts list size; **when** \"../enabled = 'true'\" makes leaf conditionally present. RFC 7950 (YANG 1.1) adds: **action** (RPC scoped to data node), **notification** (scoped event), **anydata** (opaque subtree)."
  },
  mathModelling: {
