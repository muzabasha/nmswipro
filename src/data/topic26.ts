...
export const topic26Data: TopicData = {
  ...
  storytelling: {
    ...
    technicalConnection: "**RFC 1157 SNMP Discovery**: GET sysDescr.0 (1.3.6.1.2.1.1.1.0), sysObjectID.0 (1.3.6.1.2.1.1.2.0), sysName.0 (1.3.6.1.2.1.1.5.0), sysLocation.0 (1.3.6.1.2.1.1.6.0). ifTable walk (1.3.6.1.2.1.2.2) - GET-NEXT traversal from ifIndex.1 through ifDescr, ifType (6=ethernetCsmacd, 24=softwareLoopback), ifSpeed, ifOperStatus (1=up, 2=down). **RFC 3414 SNMPv3 Discovery**: USM authentication with discovery process - send SNMP GET with msgUserName='initial' to retrieve msgAuthoritativeEngineID, msgAuthoritativeEngineBoots, msgAuthoritativeEngineTime. Subsequent requests use discovered engine parameters. **IEEE 802.1AB LLDP Discovery**: LLDP-MIB (1.0.8802.1.1.2). Walk lldpRemTable (1.0.8802.1.1.2.1.4) - for each local port, retrieve lldpRemChassisId (neighbor device MAC/ID), lldpRemPortId (neighbor port), lldpRemSysName (neighbor hostname). Topology link: (local_device, local_port) ↔ (lldpRemSysName, lldpRemPortId). **RFC 4293 IP-MIB Discovery**: ipAddressTable walk (1.3.6.1.2.1.4.34) retrieves interface IP addresses. ipv4InterfaceTable (1.3.6.1.2.1.4.28) provides IPv4 enable status per interface. **RFC 6241 NETCONF Discovery**: Client sends <hello> with capabilities. Server responds with <hello> listing supported YANG modules: <capability>urn:ietf:params:netconf:capability:yang-library:1.0?revision=2019-01-04&amp;module-set-id=abc123</capability>. Client sends get to /ietf-yang-library:modules-state for full module list with revisions and features. **RFC 8040 RESTCONF Discovery**: OPTIONS /restconf returns supported YANG modules. GET /restconf/data/ietf-yang-library:yang-library returns module-set with name, revision, namespace, location (URL to download YANG file). **Discovery Algorithm**: (1) ICMP ping sweep on IP range [start_ip, end_ip] - identify live hosts. (2) For each responding IP: attempt SNMP GET sysObjectID (timeout 5s). If success: device is SNMP-managed → continue MIB walk. If fail: attempt NETCONF SSH connection port 830. (3) SNMP MIB walk: system group + ifTable. Store device object: {ip, sysName, sysObjectID, interfaces[]}. (4) LLDP neighbor discovery: walk lldpRemTable on all discovered switches/routers. Build adjacency list: topology_links = [(dev_A, port_x, dev_B, port_y), ...]. (5) NETCONF capability exchange (parallel, 50 sessions): retrieve YANG library, store yang_capabilities per device."
  },
  mathModelling: {
