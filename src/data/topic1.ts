...
export const topic1Data: TopicData = {
  ...
  storytelling: {
    ...
    technicalConnection: "3GPP TS 23.501 §5.2 defines the 5G System Architecture: UE connects via NR air interface (TS 38.300) to gNodeB using Uu interface. gNodeB connects to 5GC via N2 (control plane, NGAP protocol per TS 38.413) and N3 (user plane, GTP-U per TS 29.281). Core Network Functions: AMF (Access and Mobility Management Function, §6.2.2) handles registration (RM-REGISTERED state machine), connection management (CM-IDLE/CM-CONNECTED states), and mobility (handover via N2 signaling). SMF (Session Management Function, §6.3.2) manages PDU Sessions with 5QI (5G QoS Identifier) values: 5QI=1 (conversational voice, 100ms latency), 5QI=5 (IMS signaling, 100ms), 5QI=9 (default bearer, best-effort). UPF (User Plane Function, §6.2.3) performs packet routing, QoS enforcement via GBR (Guaranteed Bit Rate) and MBR (Maximum Bit Rate), and N6 interface to DN. Management Plane: O-RAN Alliance M-Plane specification uses NETCONF (RFC 6241) with YANG models (o-ran-sc-du-hello-world for gNodeB configuration). Cell coverage formula (3GPP TR 38.901 §7.4.1): Maximum Coupling Loss MCL = EIRP_gNB + G_UE - Sensitivity_UE - Margin = 46 dBm + 0 dBi - (-100 dBm) - 10 dB = 136 dB. For Urban Macro (UMa) scenario: Path Loss PL = 28.0 + 22×log₁₀(d) + 20×log₁₀(f_c), where d is distance in meters, f_c is carrier frequency in GHz. At 3.5 GHz: cell edge at PL=136 dB → d ≈ 740 meters radius."
  },
  mathModelling: {
