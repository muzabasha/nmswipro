...
export const topic16Data: TopicData = {
  ...
  storytelling: {
    ...
    technicalConnection: "RFC 6241 (NETCONF 1.1) §1.1 defines protocol layers: **Transport**: SSH (RFC 6242, TCP/830), TLS (RFC 7589), BEEP (deprecated). SSH framing: chunk-encoded (RFC 6242 §4.2) or EOM marker (]]>]]>). **Messages**: <rpc>, <rpc-reply>, <notification>. **Operations** (§7-8): <get-config source=\"running|candidate|startup\"/>, <edit-config target=\"running|candidate\"> <config> <interfaces xmlns=\"...\"> <interface><name>eth0</name><enabled>false</enabled></interface> </interfaces> </config> </edit-config>, <copy-config source=\"candidate\" target=\"running\"/>, <delete-config target=\"startup\"/>, <lock target=\"candidate\"/>, <unlock target=\"candidate\"/>, <commit/> (requires :candidate:1.0 capability), <discard-changes/>, <validate source=\"candidate\"/> (requires :validate:1.0), <get filter type=\"subtree\">...</get>. **Datastores (§5.1)**: **running** (active config, always present), **candidate** (staging, requires :candidate:1.0 capability), **startup** (boot config, requires :startup:1.0). **Capabilities (§8)**: advertised in <hello> message. Base:1.0 (mandatory), :candidate:1.0 (candidate datastore), :confirmed-commit:1.0 (commit with automatic rollback timer), :rollback-on-error (atomic commit, all-or-nothing), :validate:1.0 (pre-commit validation), :url (copy-config from/to URL), :xpath:1.0 (XPath filter expressions), :notification:1.0 (event stream subscription per RFC 5277). **edit-config operations (§7.2)**: operation=\"merge\" (default, merge new data), operation=\"replace\" (replace existing node), operation=\"create\" (fail if exists), operation=\"delete\" (remove node). **Error Handling**: <rpc-error> with <error-type>protocol|rpc|application</error-type>, <error-tag>invalid-value|data-missing|lock-denied</error-tag>, <error-severity>error|warning</error-severity>, <error-path>/interfaces/interface[name='eth0']/mtu</error-path>. **Confirmed Commit (RFC 6241 §8.3.4.1)**: <commit><confirmed/><confirm-timeout>600</confirm-timeout></commit> starts a timer (600s); operator must issue <commit/> (confirming-commit) before timeout or config auto-reverts. Prevents lock-out during remote config changes."
  },
  mathModelling: {
