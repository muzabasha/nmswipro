...
export const topic43Data: TopicData = {
  ...
  virtualLab: {
    description: "Simulate service assurance monitoring by varying the probe interval and observing how detection latency and SLA report accuracy change. A shorter probe interval catches breaches faster but generates more monitoring traffic. The simulation plots estimated detection latency (in seconds) as probe interval varies from 10 seconds to the selected maximum.",
    interpretation: "Detection latency equals approximately half the probe interval on average (a breach can occur immediately after a probe or just before the next one). At 60-second probe intervals, average detection latency is 30 seconds — marginal for enterprise SLAs. At 5-minute intervals, average latency is 2.5 minutes — far too slow for 60-second breach detection requirements. Streaming telemetry (1-second intervals) achieves sub-1-second detection latency, meeting the most demanding SLA requirements.",
    parameters: [
