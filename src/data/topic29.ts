...
export const topic29Data: TopicData = {
  ...
  storytelling: {
    ...
    technicalConnection: "**RFC 7231 HTTP Methods and Semantics**: GET (§4.3.1) - safe, idempotent, cacheable. Response: 200 OK + body, 404 Not Found. POST (§4.3.3) - non-idempotent, creates resource. Response: 201 Created + Location header, 400 Bad Request. PUT (§4.3.4) - idempotent, replaces resource. 200 OK or 204 No Content. DELETE (§4.3.5) - idempotent, removes resource. 204 No Content, 404 if not exists. **RFC 6749 OAuth 2.0**: Client credentials grant (§4.4) - POST /oauth2/token body: grant_type=client_credentials&client_id={id}&client_secret={secret}. Response: {\"access_token\":\"eyJhbGc...\",\"token_type\":\"Bearer\",\"expires_in\":3600}. **RFC 8259 JSON Format**: JSON object: {\"key\":\"value\"}. Array: [{},{}]. Numbers: 123, 123.45, 1.23e10. Strings: UTF-8 encoded. **RFC 7235 Bearer Authentication**: Authorization: Bearer {access_token}. Server validates: JWT signature (HMAC-SHA256 or RSA), expiry (exp claim < now), issuer (iss claim matches expected). 401 Unauthorized + WWW-Authenticate: Bearer realm=\"NMS API\" on failure. **RFC 3986 URI Syntax**: scheme://authority/path?query#fragment. NMS example: https://nms.operator.com:443/api/v4/alarm?severity=critical&state=unacknowledged&limit=100. Query encoding: space → %20, + → %2B. **OpenAPI 3.0 Specification**: paths: /alarm: get: parameters: - name: severity, in: query, schema: {type: string, enum: [critical, major, minor, warning]}. responses: 200: content: application/json: schema: {type: array, items: {$ref: '#/components/schemas/Alarm'}}. **REST Constraints**: (1) Stateless - no session state on server. (2) Uniform interface - resources identified by URI. (3) Cacheable - GET responses cacheable with Cache-Control: max-age=300. (4) Layered system - client unaware of intermediary proxies/gateways. **HTTP/2 Multiplexing**: Single TCP connection carries multiple concurrent requests. Stream IDs identify requests. HEADERS frame + DATA frame per request. Server push capability (PUSH_PROMISE frame) for proactive resource delivery. **Content Negotiation**: Accept: application/json (client preference). Server responds with Content-Type: application/json; charset=utf-8. **HTTP Status Codes**: 2xx success (200 OK, 201 Created, 204 No Content). 4xx client error (400 Bad Request, 401 Unauthorized, 404 Not Found, 429 Too Many Requests). 5xx server error (500 Internal Server Error, 503 Service Unavailable). **Rate Limiting Headers**: X-RateLimit-Limit: 1000, X-RateLimit-Remaining: 847, X-RateLimit-Reset: 1640000000. 429 response with Retry-After: 3600."
  },
  mathModelling: {
