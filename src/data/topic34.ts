...
export const topic34Data: TopicData = {
  ...
  storytelling: {
    ...
    technicalConnection: "ONOS Intent Framework: the org.onosproject.net.intent package abstracts routing intent — a HostToHostIntent specifies source and destination hosts, and ONOS compiles the intent into flow rules via the FlowRuleService. Rollback is implemented by versioning intents: ONOS maintains intent state machine (COMPILING → INSTALLED → WITHDRAWING → WITHDRAWN). LLDP (Link Layer Discovery Protocol) is used by OpenFlow controllers for automatic topology discovery — the controller injects LLDP packets via PACKET_OUT and receives them via PACKET_IN to discover adjacencies. OpenDaylight's MD-SAL (Model-Driven Service Abstraction Layer) stores network topology in a YANG-modelled data store, enabling transactional updates with rollback semantics."
  },
  mathModelling: {
  ...
  virtualLab: {
    description: "Explore how path computation time scales with network size. Adjust the number of switches and links to see the relative Dijkstra computation time. This models the SDN controller route function's computational load during a mass rerouting event. The formula used is T_rel = (V+E) × log₂(V+1).",
    interpretation: "Path computation time grows super-linearly as both V and E increase. Notice that adding links (increasing E) has a more significant impact than adding switches at higher scales, because dense networks (E >> V) make the (V+E) term dominate. In practice, SDN controllers use incremental SPF to avoid full recomputation — but understanding the full Dijkstra baseline helps architects size controller resources and design topologies that minimise computation overhead. Fat-tree data-centre topologies intentionally limit path length (2-3 hops) to keep E manageable relative to V.",
    parameters: [
      { id: "switches", name: "Number of Switches", min: 10, max: 200, default: 50, step: 5, unit: "" },
      { id: "links", name: "Number of Links", min: 10, max: 500, default: 100, step: 10, unit: "" }
    ],
