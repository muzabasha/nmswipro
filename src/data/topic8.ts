...
export const topic8Data: TopicData = {
  ...
  storytelling: {
    ...
    technicalConnection: "RFC 3416 (SNMP Protocol Operations v2) defines query PDU structures: **GetRequest PDU** = {request-id (INT32, for correlation), error-status=0 (noError), error-index=0, variable-bindings [(OID, NULL), ...]}. Response PDU echoes request-id, returns actual values. **GetNextRequest**: retrieves lexicographically next OID. Algorithm: agent searches MIB tree depth-first from requested OID + 1. Used by snmpwalk: starts at 1.3.6.1, GetNext returns 1.3.6.1.1.1.0, next GetNext(1.3.6.1.1.1.0) returns 1.3.6.1.1.2.0, iterates until endOfMibView. **GetBulkRequest (RFC 3416 §4.2.3)**: PDU structure = {request-id, non-repeaters=N, max-repetitions=M, variable-bindings}. non-repeaters: number of scalar OIDs retrieved once. max-repetitions: number of iterations for remaining OIDs (tables). Example: GetBulk(non-repeaters=2, max-repetitions=10, varbind-list=[sysUpTime.0, sysName.0, ifDescr, ifType]) retrieves sysUpTime once, sysName once, then 10 rows of ifTable starting from ifDescr column. Efficient table retrieval: ifTable has 12 columns (ifIndex through ifSpecific). GetBulk with max-rep=48 retrieves 4 rows (48÷12=4). **SetRequest**: PDU = {request-id, error-status, error-index, [(OID, newValue), ...]}. Agent validates value type (INTEGER, OctetString, Counter32, etc.) against MIB, performs access control check (VACM view), then applies change atomically per-OID. If any OID fails, entire SET fails with error-status=genErr (5), error-index=position. No multi-OID transaction rollback in SNMP."
  },
