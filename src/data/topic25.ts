...
export const topic25Data: TopicData = {
  ...
  storytelling: {
    ...
    technicalConnection: "Alarm suppression is implemented in the NMS alarm processing pipeline between the correlation engine and the alarm console. In OpenNMS, suppression rules are configured via the correlator. In IBM Netcool Omnibus, ObjectServer automation handles suppression via @Acknowledged and @Suppressed fields. In Prometheus Alertmanager, inhibit_rules define parent-child relationships: if a parent alert is firing, inhibit child alerts from the same region. In ETSI NGMN alarm standards, the AlarmSuppressed state is a defined alarm lifecycle state. Suppressed alarms are stored in the alarm database with state='SUPPRESSED' and remain queryable for audit purposes — they are hidden from the active console but never deleted until cleared."
  },
  mathModelling: {
