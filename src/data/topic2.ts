...
export const topic2Data: TopicData = {
  ...
  storytelling: {
    ...
    technicalConnection: "ITU-T M.3010 (TMN Principles) §7 defines the 5-layer management architecture: NEL (Network Element Layer) consists of NEs exposing Q3 interface using CMIP (Common Management Information Protocol per ITU-T X.711) or SNMP. EML (Element Management Layer) aggregates NEs per vendor/technology domain. NML (Network Management Layer) provides end-to-end service view across domains. SML (Service Management Layer) maps network services to business services. BML (Business Management Layer) handles customer-facing processes. Q-interfaces: Q3 (NE↔EMS, CMIP/SNMP), Qx (EMS↔NMS, typically CORBA/SOAP), F (operator↔system, GUI/CLI). TM Forum eTOM (GB921-R, Release 23.0) extends TMN: FAB (Fulfillment-Assurance-Billing) mapped to vertical process groupings: Strategy/Infrastructure/Product (SIP) for network design, Operations (OPS) containing FCAPS functions (Fault→Assurance, Configuration→Fulfillment, Accounting→Billing), Enterprise Management (EM) for HR/finance. eTOM Level 2 processes: 1.2.3 Manage Service Problem (maps to TMN Fault Management), 1.2.4 Manage Service Quality & Availability (Performance Management), 1.3.1 Manage Service Inventory (Configuration Management). Each process has defined inputs (e.g., alarm notification), outputs (trouble ticket), roles (NOC engineer), and KPIs (MTTR). Real-world implementation: Nokia NetCracker OSS stack implements eTOM Level 1-3 processes using microservices per process group."
  },
  mathModelling: {
