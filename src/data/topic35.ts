...
export const topic35Data: TopicData = {
  ...
  storytelling: {
    ...
    technicalConnection: "**OpenTelemetry Specification v1.0**: Three signals - Metrics (Counter, Gauge, Histogram), Logs (LogRecord with timestamp, severity, body, attributes), Traces (Span with trace_id, span_id, parent_span_id, start/end timestamps, attributes[], events[]). OTLP (OpenTelemetry Protocol): gRPC/HTTP transport - ExportTraceServiceRequest with ResourceSpans[]. **RFC 5424 Syslog**: Structured format - <PRI>VERSION TIMESTAMP HOSTNAME APP-NAME PROCID MSGID [SD-ID param=value] MSG. Facility codes: 16=local0 (network devices). Severity: 0=emergency, 3=error, 6=info. **RFC 6241 NETCONF Telemetry**: <get> with <filter type='subtree'> → retrieve interfaces/interface[name='GigE0/1']/statistics/in-octets. **RFC 8639 YANG-Push**: <establish-subscription><stream>NETCONF</stream><encoding>encode-xml</encoding><periodic><period>1000</period></periodic></establish-subscription> → device pushes notifications every 1s. **gNMI Specification**: Subscribe RPC - SubscriptionList: subscription[{path: /interfaces/interface[name=*]/state/counters/in-octets, mode: SAMPLE, sample_interval: 1000000000 (nanoseconds)}]. SubscribeResponse stream: update{timestamp, path, val}. **W3C Trace Context**: traceparent header - version-trace_id-parent_id-trace_flags. Example: 00-4bf92f3577b34da6a3ce929d0e0e4736-00f067aa0ba902b7-01. Baggage header: key1=value1,key2=value2 (business context propagation). **Prometheus PromQL**: rate(interface_in_octets{device='router-1'}[5m]) - per-second rate over 5-minute window. histogram_quantile(0.95, http_request_duration_seconds_bucket) - 95th percentile latency. **Jaeger Trace Format**: Span JSON - {traceID, spanID, operationName, startTime (μs), duration (μs), tags[], logs[{timestamp, fields[]}], references[{refType: CHILD_OF, traceID, spanID}]}. **Elasticsearch Log Query**: GET /logs-*/_search {\"query\": {\"bool\": {\"must\": [{\"match\": {\"message\": \"BGP\"}}, {\"range\": {\"@timestamp\": {\"gte\": \"now-1h\"}}}]}}}. **SNR Calculation**: SNR_dB = 10 × log₁₀(P_signal / P_noise). Alert fatigue threshold: SNR < 3 dB (signal:noise ratio < 2:1)."
  },
  mathModelling: {
