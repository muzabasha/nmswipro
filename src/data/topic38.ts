...
export const topic38Data: TopicData = {
  ...
  storytelling: {
    ...
    technicalConnection: "gNMI (gRPC Network Management Interface) is defined in OpenConfig; it supports three subscription modes: ONCE (single read), POLL (on-demand), and STREAM (continuous push at specified intervals). EBPF programs are loaded via the bpf() system call and can hook into kernel tracepoints, kprobes, XDP (eXpress Data Path) for packet processing, and socket filters. Tools: bpftrace for ad-hoc kernel tracing, Cilium for Kubernetes networking with EBPF-native observability, Falco for security event detection. OpenTelemetry (OTel) is the CNCF standard for vendor-neutral instrumentation — the OTel Collector aggregates traces, metrics, and logs and exports to any backend (Jaeger, Prometheus, Loki, Datadog). IPFIX (RFC 7011) is the IETF standard for flow telemetry, extending NetFlow with flexible templates for custom field definitions."
  },
  mathModelling: {
  ...
  virtualLab: {
    description: "Explore how telemetry data volume scales with the number of devices and sampling frequency. Adjust the parameters to understand the infrastructure implications of different telemetry configurations. All calculations assume 400-byte samples — typical for gNMI streaming telemetry with 50 interface counters.",
    interpretation: "Telemetry volume grows linearly with both device count and frequency — doubling either doubles the volume. At low frequencies (1 Hz), 100 devices produce about 320 kbps — easily handled by a single Kafka topic. At high frequencies (60 Hz) for large networks (1000 devices), volume exceeds 100 Mbps — requiring distributed collectors and careful retention policies. The practical design point for production deployments is 10-30 Hz for critical interfaces (core links, peering links) and 1-5 Hz for access interfaces, using the mixed-frequency model to balance resolution against infrastructure cost.",
    parameters: [
      { id: "devices", name: "Number of Devices", min: 10, max: 1000, default: 100, step: 10, unit: "" },
      { id: "frequency", name: "Sample Frequency (Hz)", min: 1, max: 60, default: 1, step: 1, unit: " Hz" }
    ],
