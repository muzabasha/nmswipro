...
export const topic7Data: TopicData = {
  ...
  storytelling: {
    ...
    technicalConnection: "RFC 1213 (MIB-II) defines the standard SNMP data model: **OID Structure**: iso(1).org(3).dod(6).internet(1) branches into mgmt(2) [standard MIBs], private(4) [enterprise MIBs], experimental(3), security(5). Standard groups under mib-2(1): system(1), interfaces(2), at(3), ip(4), icmp(5), tcp(6), udp(7), egp(8), snmp(11). **System Group OIDs**: sysDescr.0 = 1.3.6.1.2.1.1.1.0 (text description), sysObjectID.0 = 1.3.6.1.2.1.1.2.0 (vendor enterprise OID), sysUpTime.0 = 1.3.6.1.2.1.1.3.0 (TimeTicks since boot), sysContact.0/sysName.0/sysLocation.0 (administratively set strings). **Interfaces Group (RFC 2863)**: ifTable = 1.3.6.1.2.1.2.2, indexed by ifIndex. Key columns: ifDescr (e.g., \"GigabitEthernet0/0\"), ifType (INTEGER, 6=ethernetCsmacd, 131=tunnel), ifMtu, ifSpeed (Gauge32, bps), ifPhysAddress (MAC), ifAdminStatus (up/down/testing), ifOperStatus (up/down/testing/unknown/dormant/notPresent/lowerLayerDown), ifInOctets/ifOutOctets (Counter32, wraps at 2^32), ifInErrors/ifOutErrors. **SNMP PDU structure (RFC 3416)**: PDU = { request-id (INT32), error-status (0=noError, 1=tooBig, 2=noSuchName, 5=genErr), error-index (INT, identifies failing varbind), variable-bindings [(OID, value), ...] }. GetBulkRequest: non-repeaters=N (scalars fetched once), max-repetitions=M (table rows). Example: GetBulk with non-repeaters=1 (sysUpTime), max-repetitions=20 (ifTable) retrieves sysUpTime once + 20 ifTable rows. **TRAP PDU (RFC 1215)**: enterprise OID, generic-trap (0=coldStart, 1=warmStart, 2=linkDown, 3=linkUp, 4=authenticationFailure), specific-trap (vendor-defined), time-stamp, varbind-list. SNMPv2 Trap (RFC 3416): uses snmpV2-Trap PDU, includes snmpTrapOID.0 varbind identifying trap type."
  },
