...
export const topic3Data: TopicData = {
  ...
  storytelling: {
    ...
    technicalConnection: "3GPP TS 32.101 (Telecom Management Principles) §4.2 defines the EMS/NMS architecture hierarchy: NEs (eNB/gNB) expose Itf-N interface (NETCONF/YANG per 3GPP TS 28.541 or vendor-proprietary SNMP). EMS (Element Management System, e.g., Ericsson ENM, Nokia NetAct) aggregates per vendor: Ericsson ENM manages RBS (Radio Base Stations) via proprietary CorbaNET; Nokia NetAct manages gNodeBs via REST/YANG. NMS (Network Management System) sits above multiple EMSes at the Service Management Layer. Alarm flow: NE generates alarm (3GPP TS 32.111-2 Alarm IRP, 6 severity levels: Critical/Major/Minor/Warning/Indeterminate/Cleared) → EMS normalizes vendor-specific format → NMS correlates alarms across domains. For example: when a transmission link fails, Microwave NE (Huawei RTN) raises linkDown alarm; Ericsson eNB detects X2 interface loss; NMS correlates both as single root-cause event. SBI/NBI architecture: NMS Southbound uses 3GPP IRPs (Integration Reference Points): AlarmIRP (TS 32.111-2), PerfIRP (TS 32.401), ConfigIRP (TS 32.600). Northbound exposes TMF Open APIs: TMF642 (Alarm Management API) returns JSON: {\"alarmType\":\"equipmentAlarm\", \"perceivedSeverity\":\"CRITICAL\", \"probableCause\":\"powerProblem\", \"affectedObject\":\"/nrm:SubNetwork=ManagedElement=gNB001\"}."
  },
  mathModelling: {
  ...
  virtualLab: {
    description: "You are a NOC operations lead configuring alarm policies for a multi-domain network. Your task: determine the alarm aggregation ratio achieved by the NMS as you add EMS domains. Adjust the number of EMS domains and the raw alarm volume each domain generates. The NMS correlates approximately 15% of raw alarms into root-cause alarms. The chart shows the aggregation compression ratio — your goal is to verify that the NMS keeps the operators' actionable alarm volume manageable as the network expands.",
    interpretation: "As managed EMS domains increase, total alarm volume grows linearly, but the aggregation ratio stays stable around 6.67:1 (each raw alarm compressed to 0.15 actionable alarms). With 5 domains each generating 200 raw alarms, total = 1,000 raw → 150 actionable — still manageable for a single NOC shift. This consistent compression validates the hierarchical EMS→NMS model: EMS pre-filters per-domain noise locally; the NMS applies cross-domain correlation to produce a stream operators can actually work through. A ratio above 10:1 means operators are drowning in noise — increase correlation rule coverage to bring it down.",
    parameters: [
