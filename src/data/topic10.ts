...
export const topic10Data: TopicData = {
  ...
  storytelling: {
    ...
    technicalConnection: "RFC 1215 (Trap Convention for SNMPv1) defines trap PDU structure: **SNMPv1 Trap-PDU** = {enterprise (OID, e.g., 1.3.6.1.4.1.9 for Cisco), agent-addr (IP of sending device), generic-trap (INTEGER 0-6: coldStart=0, warmStart=1, linkDown=2, linkUp=3, authenticationFailure=4, egpNeighborLoss=5, enterpriseSpecific=6), specific-trap (INT, vendor-defined if generic-trap=6), time-stamp (TimeTicks, sysUpTime.0 at event occurrence), variable-bindings [(OID, value), ...]}. **SNMPv2c/v3 Trap-PDU (RFC 3416 §4.2.6)**: unified PDU type = Trap-PDU or Notification-PDU. Mandatory varbinds: sysUpTime.0 (first), snmpTrapOID.0 (second, identifies trap type using OID notation e.g., 1.3.6.1.6.3.1.1.5.3=linkDown). Additional varbinds carry event details. Example linkDown trap: varbind[2]=snmpTrapOID.0=1.3.6.1.6.3.1.1.5.3 (linkDown OID), varbind[3]=ifIndex=4, varbind[4]=ifAdminStatus=down(2), varbind[5]=ifOperStatus=down(2). **INFORM-PDU**: Manager-to-Manager notification with acknowledgment. PDU includes request-id, sender retransmits at configured interval (default 1s, 3 retries) until receiving SNMPv2-Response. Delivery reliability: TRAP (UDP, no ACK) ≈ 95% on 5% lossy link (1 PDU lost), INFORM (3 retries) ≈ 99.875% (probability all 4 attempts lost = 0.05^4). Transport: UDP/162 for traps/informs received by NMS. Agent configuration: trap-target (NMS IP), community/USM credentials, trap-enable list (which traps to send)."
  },
  mathModelling: {
