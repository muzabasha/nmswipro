...
export const topic18Data: TopicData = {
  ...
  storytelling: {
    ...
    technicalConnection: "RFC 8040 RESTCONF Protocol: **Base URL Structure** (§3.1): https://{host}:{port}/restconf/. Well-known URI: /.well-known/host-meta (§3.1) returns <Link rel='restconf' href='/restconf'/>. **Data Resources** (§3.3): /restconf/data/{module-name}:{identifier}/{identifier}... **HTTP Method Semantics** (§4): GET (§4.1) retrieves resource, returns 200 OK + JSON/XML body or 404 Not Found. HEAD returns headers without body. POST (§4.4) creates resource at target collection: POST /restconf/data/ietf-interfaces:interfaces with JSON body containing new interface → 201 Created with Location: /restconf/data/ietf-interfaces:interfaces/interface=eth0 header. Returns 409 Conflict if resource exists. PUT (§4.5) replaces resource: PUT /restconf/data/ietf-interfaces:interfaces/interface=eth0 with full interface JSON → 204 No Content (updated) or 201 Created (new). Deletes child nodes not in PUT body. PATCH (§4.6) merges partial changes: PATCH /restconf/data/ietf-interfaces:interfaces/interface=eth0 with JSON { 'ietf-interfaces:interface': { 'description': 'Updated' } } → 204 No Content. Only specified leafs modified. RFC 8072 YANG Patch (application/yang-patch+json) provides ordered operations: { 'ietf-yang-patch:yang-patch': { 'patch-id': 'patch-1', 'edit': [{ 'edit-id': '1', 'operation': 'merge', 'target': '/ietf-interfaces:interfaces/interface=eth0', 'value': {...} }] } }. DELETE (§4.7) removes resource: DELETE /restconf/data/ietf-interfaces:interfaces/interface=eth0 → 204 No Content or 404 Not Found. **Query Parameters** (§4.8): depth (tree depth limit), fields (field projection — retrieves subset of leafs), filter (XPath 1.0 filter), with-defaults (RFC 6243 default handling). Example: GET /restconf/data/ietf-interfaces:interfaces?fields=interface(name;oper-status) returns only name and oper-status leafs. **Operations** (§3.6): POST /restconf/operations/{module}:{rpc-name} with input in body → returns output. **YANG Library** (RFC 8525): GET /restconf/data/ietf-yang-library:yang-library returns module-set with name, revision, namespace, feature, deviation. **Error Responses** (§7): JSON errors: { 'ietf-restconf:errors': { 'error': [{ 'error-type': 'application', 'error-tag': 'invalid-value', 'error-path': '/ietf-interfaces:interfaces/interface[name=\"eth0\"]/mtu', 'error-message': 'MTU value 10000 exceeds max 9216' }] } }. Status codes: 400 Bad Request, 401 Unauthorized, 403 Forbidden, 404 Not Found, 405 Method Not Allowed, 409 Conflict, 412 Precondition Failed, 413 Payload Too Large, 415 Unsupported Media Type, 500 Internal Server Error. **Media Types** (§5): application/yang-data+json (RFC 7951 JSON encoding), application/yang-data+xml. **Security**: TLS 1.2+ mandatory (§2.1). HTTP Basic Auth, OAuth2 Bearer tokens, or TLS client certificates."
  },
  mathModelling: {
