...
export const topic13Data: TopicData = {
  ...
  storytelling: {
    ...
    technicalConnection: "Model-Driven Management separates the data model (YANG) from the transport protocol (NETCONF/RESTCONF) and the device (NE). The NMS uses YANG models to generate configuration structures, validate them before sending, and understand device capabilities via YANG capability exchange. Key components: YANG model repository, NETCONF server (on device), NETCONF client (on NMS), candidate datastore, commit/rollback."
  },
  mathModelling: {
