...
export const topic37Data: TopicData = {
  ...
  storytelling: {
    ...
    technicalConnection: "**TM Forum TMF641 Service Ordering**: POST /tmf-api/serviceOrderingManagement/v4/serviceOrder body: {\"description\":\"MPLS L3VPN\",\"serviceOrderItem\":[{\"service\":{\"serviceSpecification\":{\"id\":\"L3VPN-100Mbps\"},\"serviceCharacteristic\":[{\"name\":\"bandwidth\",\"value\":\"100Mbps\"},{\"name\":\"customer_id\",\"value\":\"CUST-7829\"}]}}]}. Response: {\"id\":\"SO-12345\",\"state\":\"acknowledged\"}. Link to observability: customer_id propagated in traces → service impact correlation. **TM Forum TMF640 Service Activation**: GET /tmf-api/serviceActivationAndConfiguration/v4/service/{id} → {\"state\":\"active\",\"startDate\":\"2025-01-15T10:00:00Z\"}. SLA measurement: (activation_time - order_time) < SLA_target (e.g., 4 hours). **TM Forum TMF642 Alarm with Service Impact**: GET /tmf-api/alarmManagement/v4/alarm?affectedService=SVC-7829 → alarms filtered by business service. Each alarm: {\"id\":\"ALM-456\",\"affectedService\":[{\"id\":\"SVC-7829\",\"@referredType\":\"Service\"}],\"estimatedImpact\":{\"customersAffected\":2847,\"revenueImpact\":{\"amount\":15000,\"currency\":\"USD\"}}}. **OpenTelemetry Baggage**: W3C Baggage header - customer_id=CUST-7829,transaction_value=1500.00,order_id=SO-12345. Propagated through all spans → business context in every trace. **RFC 8639 YANG-Push for SLA Monitoring**: Subscription to /ietf-interfaces:interfaces/interface[name='GigE0/1']/statistics/out-octets at 1s interval → stream to Prometheus → PromQL: rate() < CIR_threshold triggers SLA breach alert. **ITU-T E.800 QoS Metrics**: Availability A = (total_time - downtime) / total_time × 100. MTBF (Mean Time Between Failures) = total_uptime / failure_count. MTTR (Mean Time To Repair) = total_downtime / failure_count. SLA credit formula: credit = MRC × breach_percentage × penalty_multiplier. **Business Impact Calculation**: I_down = R_rate × T_down + P_sla. R_rate varies by time-of-day: peak (09:00-17:00) = $8,000/min, off-peak = $2,000/min. P_sla triggered when cumulative_downtime_month > SLA_threshold (43.8 min for 99.9%). **SLA Compliance Dashboard**: Grafana panels - (1) Availability % per customer (Prometheus query: (total_time - downtime) / total_time). (2) Downtime budget remaining (SLA_threshold - cumulative_downtime_month). (3) Projected penalty exposure (if current month trend continues). (4) Top 5 customers nearest to SLA breach."
  },
  mathModelling: {
