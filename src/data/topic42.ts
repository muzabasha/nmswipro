...
export const topic42Data: TopicData = {
  ...
  storytelling: {
    ...
    technicalConnection: "Service ordering workflow: Customer submits order via BSS portal → TMF622 POST /productOrder → OSS decomposes to resource orders → domain provisioning APIs (NETCONF for IP/MPLS, ONAP SO for VNFs, REST for cloud) → completion callbacks update order state. Order states follow TMF622: acknowledged, rejected, pending, held, inProgress, cancelled, completed, failed, partial. Compensating transactions implement the saga pattern: each forward action has a corresponding rollback action registered before execution. ONAP's BPMN-based Service Orchestrator (SO) coordinates the workflow, retrying transient failures automatically before invoking compensating actions."
  },
  mathModelling: {
  ...
  virtualLab: {
    description: "Simulate how service order provisioning time scales with the number of provisioning domains (steps) and the average per-step duration. Compare synchronous (total time = sum of all steps) vs asynchronous parallel (total time = slowest step) execution models.",
    interpretation: "With synchronous chaining, provisioning time grows linearly with the number of domains — making it impractical for complex multi-domain services. With asynchronous parallel execution (where independent steps are parallelised), total time is bounded by the slowest single step regardless of how many domains are involved. This demonstrates why modern service orchestrators parallelise independent provisioning steps rather than executing them sequentially.",
    parameters: [
