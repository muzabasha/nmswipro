...
export const topic40Data: TopicData = {
  ...
  storytelling: {
    ...
    technicalConnection: "**Prophet Time-Series Forecasting** (Meta, open-source): Additive model y(t) = g(t) + s(t) + h(t) + ε_t where g(t)=piecewise linear/logistic trend, s(t)=Fourier series seasonality (daily: 10 terms, weekly: 3 terms), h(t)=holidays/events, ε_t=error term. Training: Stan MCMC sampling for parameter estimation. Prediction: 95% confidence interval via MCMC posterior samples. Network capacity forecasting: train on 180 days hourly bandwidth, forecast 90 days ahead, alert when upper 95% CI crosses 80% utilization. **Isolation Forest Algorithm** (PyOD library): Unsupervised anomaly detection — builds random trees partitioning feature space, anomalies require fewer splits to isolate. Score: s(x,n) = 2^(-E(h(x))/c(n)) where h(x)=path length to isolate x, c(n)=average path length for n samples. Threshold: s>0.6 flags anomaly. Network application: 8 features [CPU%, mem%, bw_in, bw_out, pkt_loss%, latency_ms, error_rate, session_count]. Train on 90 days normal operation, score every 10s. **LSTM Autoencoder Architecture**: Encoder: 3 LSTM layers [128, 64, 32 units] → latent representation (32-dim). Decoder: 3 LSTM layers [32, 64, 128 units] → reconstruction. Loss: MSE between input and reconstruction. Training: 60-day window × 8 metrics per device, batch=64, epochs=50. Inference: reconstruction error > 99th percentile of training errors → anomaly. Network application: detects temporal anomalies (CPU spike + BGP flap co-occurrence) that single-metric thresholds miss. **MAPE Calculation Details**: MAPE = (1/n) Σ|A_i - F_i|/A_i × 100%. Acceptable thresholds: <10% excellent, 10-20% good, >30% unacceptable. Handling A_i=0: use sMAPE (symmetric MAPE) = (1/n) Σ 2|A_i-F_i|/(|A_i|+|F_i|) × 100%. Prophet forecast validation: rolling window cross-validation — train on [t-180d, t-7d], predict [t-7d, t], compute MAPE, slide window forward 1 day, average over 30 windows. **Root Cause Analysis via Causal Inference**: Granger causality test — X Granger-causes Y if past values of X improve prediction of Y beyond past values of Y alone. Test: VAR(p) model, F-statistic for null hypothesis 'X coefficients = 0'. Network RCA: if config_change(t-5min) Granger-causes BGP_flap(t) with p<0.01, flag config_change as root cause. **Graph Neural Network for Alert Correlation**: GNN architecture: node=alarm, edge=topological dependency. Node features: [alarm_type_embedding, severity, timestamp, device_location]. Message passing: 3 GCN layers aggregate neighbor features. Output: cluster ID per alarm (alarms with same cluster ID share root cause). Training: supervised on 6 months labelled incidents (manual RCA ground truth). **MLflow Model Registry**: Experiment tracking — log hyperparameters (Prophet changepoint_prior_scale=0.05), metrics (MAPE=8.3%), artifacts (trained model.pkl). Model versioning: Prophet_capacity_v1.2 (production), Prophet_capacity_v1.3 (staging). Deployment: REST API endpoint /predict with input {device_id, metric_name, horizon_days} returns {forecast:[], upper_bound:[], lower_bound:[]}. **Prometheus Alerting with Anomaly Detection**: anomaly_detector sidecar queries Prometheus every 30s, runs Isolation Forest inference on last 10-min window, pushes anomaly scores to custom metric 'anomaly_score{device,metric}'. Alert rule: 'anomaly_score > 0.65 for 2 minutes' → PagerDuty. Reduces false positives by 70% vs static thresholds."
  },
  mathModelling: {
  ...
  virtualLab: {
    description: "You are evaluating a capacity forecast model for network growth planning. The acceptable MAPE threshold is 15%. Your task: determine the maximum allowable per-step forecast error percentage to keep MAPE under 15% by time step 5 (a typical quarterly planning horizon). Adjust the forecast error percentage and baseline actual value. The chart shows MAPE growing across time steps — systematic bias compounds, so even small per-step errors become large by later time steps. Find the error budget that keeps MAPE under 15% through your planning horizon.",
    interpretation: "At 10% per-step error, MAPE at step 5 is 50% — far above the 15% acceptable threshold. To stay under 15% at step 5, per-step error must be under 3%. This means the forecast model must be extremely accurate in early steps, or it must be retrained frequently. The key insight: systematic bias (consistent over- or under-estimation) is far worse than random noise because it compounds over the forecast horizon. Acceptable MAPE (< 10-15%) requires models that are retrained at least every 3-4 time steps as growth patterns evolve.",
    parameters: [
      { id: "actual", name: "Actual Value (baseline)", min: 10, max: 200, default: 100, step: 10, unit: "" },
      { id: "forecast_error", name: "Forecast Error (%)", min: 1, max: 50, default: 10, step: 1, unit: "%" }
    ],
