...
type SectionColor = 'blue' | 'amber' | 'rose' | 'green' | 'purple' | 'orange' | 'cyan' | 'indigo' | 'emerald' | 'teal';
...
const colorMap: Record<SectionColor, { tab: string; active: string; ring: string; bg: string; border: string; title: string; icon: string }> = {
  ...
  teal: { tab: 'text-teal-600', active: 'bg-teal-600 text-white', ring: 'ring-teal-500', bg: 'bg-teal-50 dark:bg-teal-900/20', border: 'border-teal-200 dark:border-teal-800', title: 'text-teal-800 dark:text-teal-300', icon: 'text-teal-500' },
};
...
function TopicContent({ data, prevTopic, nextTopic }: { data: TopicData; prevTopic: { id: string; name: string; unit: string } | null; nextTopic: { id: string; name: string; unit: string } | null }) {
  ...
  const activeSections = useMemo(() => {
    const base = [...SECTIONS] as Array<{ id: string; label: string; fullLabel: string; icon: any; color: SectionColor }>;
    if (data.commands) {
      base.push({ id: 'commands', label: 'Commands', fullLabel: 'CLI Commands Reference', icon: Keyboard, color: 'teal' });
    }
    return base;
  }, [data]);
    ...
    <div className="max-w-5xl mx-auto space-y-4 sm:space-y-6 pb-20 sm:pb-24">
      ...
      {(() => {
        const sectionContent = (
          ...
          {/* ════════════════════════════════════════════════
              SECTION 9 — Commands (Dynamic)
          ════════════════════════════════════════════════ */}
          {activeSection === 9 && data.commands && (() => {
            const c = colorMap.teal;
            return (
              <div className="space-y-4 sm:space-y-5">
                <div className={`rounded-2xl p-4 sm:p-5 ${c.bg} border ${c.border}`}>
                  <div className="flex items-center gap-2 sm:gap-3 mb-1">
                    <div className={`p-1.5 sm:p-2 rounded-xl bg-teal-100 dark:bg-teal-900/40`}>
                      <Keyboard className={`w-4 h-4 sm:w-5 sm:h-5 ${c.icon}`} />
                    </div>
                    <h2 className={`text-lg sm:text-xl font-bold ${c.title}`}>CLI Commands Reference</h2>
                  </div>
                  <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 ml-9 sm:ml-11">Essential commands for network management and troubleshooting</p>
                </div>
                
                <div className="space-y-4">
                  {data.commands.map((cmd) => (
                    <div key={cmd.id} className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 overflow-hidden">
                      <div className="px-5 py-3 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                        <div className="flex items-center gap-3">
                          <span className="font-mono font-bold text-teal-600 dark:text-teal-400 text-lg">{cmd.name}</span>
                          <span className="px-2 py-0.5 rounded-full bg-slate-200 dark:bg-slate-700 text-[10px] font-semibold text-slate-600 dark:text-slate-300 uppercase tracking-wider">{cmd.category}</span>
                        </div>
                        <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
                          <Layers size={14} />
                          <span>{cmd.osiLayer}</span>
                        </div>
                      </div>
                      <div className="p-4 sm:p-5 space-y-4">
                        <p className="text-sm font-medium text-slate-800 dark:text-slate-200">{cmd.oneLiner}</p>
                        
                        <div className="grid sm:grid-cols-2 gap-4">
                          <div className="space-y-1">
                            <span className="text-xs font-semibold text-slate-500 uppercase">Protocol</span>
                            <p className="text-sm text-slate-700 dark:text-slate-300">{cmd.protocol}</p>
                          </div>
                          <div className="space-y-1">
                            <span className="text-xs font-semibold text-slate-500 uppercase">Syntax</span>
                            <code className="block text-xs font-mono bg-slate-100 dark:bg-slate-800 text-pink-600 dark:text-pink-400 px-2 py-1 rounded">{cmd.syntax}</code>
                          </div>
                        </div>

                        <div className="space-y-2">
                          <span className="text-xs font-semibold text-slate-500 uppercase">Key Options</span>
                          <div className="grid gap-2">
                            {cmd.options.map((opt, i) => (
                              <div key={i} className="flex items-start gap-2 text-sm">
                                <code className="shrink-0 font-mono text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-900/20 px-1.5 py-0.5 rounded text-xs">{opt.flag}</code>
                                <span className="text-slate-600 dark:text-slate-400 leading-relaxed">{opt.description}</span>
                              </div>
                            ))}
                          </div>
                        </div>
                        ...
                        <div className="space-y-2">
                          <span className="text-xs font-semibold text-slate-500 uppercase">Sample Output & Interpretation</span>
                          <div className="rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                            <pre className="p-3 bg-slate-900 text-slate-300 text-xs font-mono overflow-x-auto whitespace-pre-wrap">{cmd.sampleOutput}</pre>
                            <div className="p-3 bg-indigo-50 dark:bg-indigo-900/10 border-t border-indigo-100 dark:border-indigo-900/30">
                              <p className="text-sm text-indigo-800 dark:text-indigo-300 leading-relaxed">{cmd.outputInterpretation}</p>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="pt-4 border-t border-slate-100 dark:border-slate-800">
                          <div className="flex items-start gap-2">
                            ...
                            <div>
                              <span className="block text-xs font-semibold text-slate-500 mb-1">Network Management Context</span>
                              <p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed">{cmd.nmContext}</p>
                            </div>
                          </div>
                        </div>
                        
                        {cmd.windowsEquivalent && (
                          <div className="mt-2 text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
                            ...
                            <code className="font-mono bg-slate-100 dark:bg-slate-800 px-1 rounded">{cmd.windowsEquivalent}</code>
                          </div>
                        )}
                      </div>
                    </div>
      ...
      if (presentMode) {
        const SecIconS = activeSections[activeSection].icon;
        return (
          <PresentationMode
            currentSlide={activeSection}
            totalSlides={total}
            sectionLabel={activeSections[activeSection].fullLabel}
            sectionColor={activeSections[activeSection].color}
      ...
      <div className="flex items-center justify-between pt-4 border-t border-slate-200 dark:border-slate-800">
        ...
        {activeSection < activeSections.length - 1 ? (
          <button
            onClick={() => setActiveSection(Math.min(activeSections.length - 1, activeSection + 1))}
            className="flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-medium
