  ...
  return (
    <div className="flex h-screen bg-[var(--background)] text-[var(--foreground)] font-sans transition-colors duration-300">
      ...
      <div className={`flex-1 flex flex-col overflow-hidden transition-all duration-300 ease-in-out ${sidebarOpen ? 'lg:ml-64' : 'lg:ml-0'}`}>
        {/* Header */}
        <header className="h-16 glass border-b border-[var(--border)] px-4 md:px-6 flex items-center gap-2 z-40 relative">
          <button onClick={toggleSidebar} className="btn-ghost p-2 shrink-0" title={sidebarOpen ? 'Close sidebar' : 'Open sidebar'}>
            {sidebarOpen ? <X size={18} /> : <Menu size={18} />}
          </button>

          <Link to="/" className="btn-ghost p-2 shrink-0" title="Home">
          ...
          <button onClick={toggleTheme} className="btn-ghost p-2 rounded-full shrink-0" title="Toggle theme">
